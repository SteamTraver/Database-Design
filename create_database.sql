...
CREATE SCHEMA IF NOT EXISTS `test_create` DEFAULT CHARACTER SET utf8 ;

CREATE TABLE IF NOT EXISTS `test_create`.`anju_user` (
  ...
  `wechatid` VARCHAR(50) CHARACTER SET 'utf8' NULL DEFAULT NULL COMMENT 'wechatid,记录用户的微信id。用于登录验证。可空，但是唯一。',
  PRIMARY KEY (`id`),
  UNIQUE INDEX `phone_UNIQUE` (`phone` ASC) VISIBLE,
  UNIQUE INDEX `wechatid_UNIQUE` (`wechatid` ASC) VISIBLE)
ENGINE = InnoDB
AUTO_INCREMENT = 3
DEFAULT CHARACTER SET = utf8
COMMENT = '用户表。数据库核心表。记录用户的相关各种信息。部分用户可以选填，部分必须填。具体看字段注释。';

CREATE TABLE IF NOT EXISTS `test_create`.`device` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键。类型为int auto increament usigned',
  ...
  INDEX `anju_user_id_to_device1_idx` (`anju_user_id` ASC) VISIBLE,
  CONSTRAINT `anju_user_id_to_device1`
    FOREIGN KEY (`anju_user_id`)
    REFERENCES `test_create`.`anju_user` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8
...
CREATE TABLE IF NOT EXISTS `test_create`.`friend` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键。类型为int auto increament usigned',
  `create` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间。类型为datetime,精确到时分秒。',
  `update` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间。类型为datetime,精确到时分秒。',
  `anju_user_id` INT(10) UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `test_create`.`stored_image` (
  `id` INT(11) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键。类型为int auto increament usigned',
  `url` VARCHAR(100) CHARACTER SET 'utf8' NULL DEFAULT NULL COMMENT '图片存储在服务器上的位置。长度控制在100个字符。2019/9/18日修改。',
...
CREATE TABLE IF NOT EXISTS `test_create`.`stored_image_thumb` (
  `id` INT(11) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键。类型为int auto increament usigned',
  `url` VARCHAR(100) CHARACTER SET 'utf8' NULL DEFAULT NULL COMMENT '预览图在服务器上的存储位置。9/18日修改长度限制为100.',
  ...
  CONSTRAINT `image_id_to_image_thumb1`
    ...
    REFERENCES `test_create`.`stored_image` (`id` , `anju_user_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8
...
CREATE TABLE IF NOT EXISTS `test_create`.`operationlog` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键。类型为int auto increament usigned',
  ...
  CONSTRAINT `terminal_id_to_operationlog1`
    ...
    REFERENCES `test_create`.`terminal` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS `test_create`.`terminal` (
  `id` INT(11) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键。类型为int auto increament usigned',
...
CREATE TABLE IF NOT EXISTS `test_create`.`user_has_terminal` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键。类型为int auto increament usigned',
...
CREATE TABLE IF NOT EXISTS `test_create`.`stored_video` (
  `id` INT(11) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键。类型为int auto increament usigned',
  `url` VARCHAR(100) CHARACTER SET 'utf8' NULL DEFAULT NULL COMMENT '视频文件存储在服务器上的位置。2019/9/18日修改。',
...
CREATE TABLE IF NOT EXISTS `test_create`.`stored_video_thumb` (
  `id` INT(11) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键。类型为int auto increament usigned',
  `url` VARCHAR(100) CHARACTER SET 'utf8' NULL DEFAULT NULL COMMENT '预览图在服务器上的存储位置。2019/9/18日修改。',
  `name` VARCHAR(45) CHARACTER SET 'utf8' NULL DEFAULT NULL COMMENT '文件名。',
  `create` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间。',
  `update` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间。类型为datetime，精确到时分秒。',
  `type` VARCHAR(5) CHARACTER SET 'utf8' NULL DEFAULT NULL COMMENT '预览图文件格式。',
  ...
  CONSTRAINT `video_id_to_video_thumb1`
    ...
    REFERENCES `test_create`.`stored_video` (`id` , `anju_user_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8
...
CREATE TABLE IF NOT EXISTS `test_create`.`group` (
...
CREATE TABLE IF NOT EXISTS `test_create`.`device_has_group` (
  ...
  CONSTRAINT `device_id_to_device_has_group1`
    ...
    REFERENCES `test_create`.`device` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `group_id_to_device_has_group1`
    FOREIGN KEY (`group_id`)
    REFERENCES `test_create`.`group` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8
...
CREATE TABLE IF NOT EXISTS `test_create`.`living_video` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键。',
  `url` VARCHAR(100) CHARACTER SET 'utf8' COLLATE 'utf8_general_ci' NULL DEFAULT NULL COMMENT '设备的推流地址。',
  `create` DATETIME NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间。',
  `update` DATETIME NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间.',
  `device_id` INT(10) UNSIGNED NOT NULL,
  PRIMARY KEY (`id`, `device_id`),
  INDEX `device_id_to_living_video1_idx` (`device_id` ASC) VISIBLE,
  CONSTRAINT `device_id_to_living_video1`
    FOREIGN KEY (`device_id`)
    REFERENCES `test_create`.`device` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8
COMMENT = '该表记录用户的设备在推流时相关信息。与设备表是一对一关系。';

CREATE TABLE IF NOT EXISTS `test_create`.`shared_device` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键。',
  ...
  INDEX `friend_id_to_shared_device1_idx` (`friend_id` ASC, `friend_anju_user_id` ASC, `friend_anju_user_id1` ASC) VISIBLE,
  INDEX `device_id_to_shared_device1_idx` (`device_id` ASC) VISIBLE,
  CONSTRAINT `friend_id_to_shared_device1`
    FOREIGN KEY (`friend_id` , `friend_anju_user_id` , `friend_anju_user_id1`)
    REFERENCES `test_create`.`friend` (`id` , `anju_user_id` , `anju_user_id1`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `device_id_to_shared_device1`
    FOREIGN KEY (`device_id`)
    REFERENCES `test_create`.`device` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8
COMMENT = '这个表记录好友之间的共享设备。该表的更新，不能通过触发器自动更新。';
